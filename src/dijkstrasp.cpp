...
//             // cout << titles_matrix[i][j] << " ";
//             if (j < (col-1)) {
//                 titles_costs_it = titles_costs.find(titles_matrix[i][j+1]);
...
//             accumulated_cost = titles_costs_it->second + adjacent_edges_it->second;
...
//             if (adjacent_edges_it->first ==  (end_row*col+end_col)) loop = false;  // HARD CODEEEEEE. CHANGEEEEE!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
// cout << "\n\n" << accumulated_cost_to_target << endl; 
// for (path_it = path.begin(); path_it != path.end(); path_it++) {
